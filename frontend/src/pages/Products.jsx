  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 flex items-center">
              <Package className="mr-2 h-6 w-6 text-indigo-600 flex-shrink-0" />
              Products
            </h1>
          ...
          {canManage && (
            <button
              ...
              className="inline-flex items-center justify-center px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors shadow-sm font-medium w-full sm:w-auto"
            >
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4">
          <div className="flex items-center gap-3 rounded-xl border border-emerald-100 bg-emerald-50 p-4 shadow-sm">
            ...
            <div className="min-w-0">
              <p className="text-xs font-bold uppercase tracking-wider text-emerald-700 truncate">Total Products</p>
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <select
            ...
            className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500"
          >
      ...
      <div className="hidden md:block bg-white rounded-xl card-shadow border border-gray-100 overflow-hidden">
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={canManage ? 6 : 5} className="px-6 py-12 text-center">
                    ...
                    <p className="mt-1 text-sm text-gray-500">Adjust your filters or add a new product.</p>
                  </td>
                </tr>
              ) : (
        ...
        {/* Pagination */}
        {totalPages > 1 && (
          <div className="px-6 py-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-2 bg-gray-50">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-xs text-gray-500">
                    ...
                    <span>{product.gst_type || 'GST'} {Number(product.gst_percentage || 0).toFixed(2)}%</span>
                  </div>
                  {product.description && (
                    <p className="mt-1 text-xs text-gray-400 truncate">{product.description}</p>
                  )}
                </div>
              </div>
              {canManage && (
                <div className="mt-3 pt-3 border-t border-gray-100 flex justify-end">
                  <button
                    onClick={() => handleOpenModal(product)}
                    className="inline-flex items-center gap-1 text-xs font-medium text-indigo-600 hover:text-indigo-900 px-3 py-1.5 rounded-lg hover:bg-indigo-50 transition-colors"
                  >
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between pt-2">
            <p className="text-xs text-gray-500">
              Page {page} of {totalPages} · {meta.total} products
            </p>
            <div className="flex gap-2">
              <button
                onClick={() => setPage(p => Math.max(1, p - 1))}
