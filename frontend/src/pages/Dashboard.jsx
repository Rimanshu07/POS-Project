      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {cards.map((card, idx) => (
          card.isButton ? (
            ...
            >
              {card.icon}
              <span className="mt-2 text-white font-bold text-lg tracking-wide">{card.title}</span>
            </div>
          ) : (
            <div key={idx} className="bg-white rounded-xl card-shadow p-5 flex items-center gap-5 border border-gray-100 hover:shadow-md transition-modern group">
              <div className={`${card.bgColor} w-16 h-16 rounded-xl flex items-center justify-center flex-shrink-0 border transition-modern group-hover:scale-105`}>
                {card.icon}
              </div>
              <div>
                <p className="text-sm font-semibold text-gray-500 uppercase tracking-wider">{card.title}</p>
                <p className={`text-2xl font-extrabold mt-1 ${card.textColor}`}>{card.value}</p>
              </div>
            </div>
          )
        ))}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-gray-600">
            ...
            <tbody>
              {isLoadingOrders ? (
                <tr>
                  <td colSpan="9" className="px-6 py-8 text-center text-gray-500">Loading recent sales...</td>
                </tr>
              ) : ordersData?.orders?.length > 0 ? (
                ordersData.orders.map((order) => (
                  <tr key={order.id} className="border-b border-gray-50 hover:bg-indigo-50/30 transition-modern">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="font-medium text-gray-900">{format(new Date(order.created_at), 'MM/dd/yyyy')}</div>
                      <div className="text-xs text-gray-500 mt-0.5">{format(new Date(order.created_at), 'hh:mm a')}</div>
                    </td>
                    <td className="px-6 py-4 font-semibold text-gray-700">{order.order_number}</td>
                    <td className="px-6 py-4 text-center font-medium">{order.items?.length || 0}</td>
                    <td className="px-6 py-4 text-indigo-600 font-bold">₹{order.total_amount}</td>
                    ...
                    <td className="px-6 py-4 text-emerald-600 font-bold">₹0.00</td>
                ...
                <tr>
                  <td colSpan="9" className="px-6 py-8 text-center text-gray-500">No recent sales found</td>
                </tr>
              )}
            </tbody>
          </table>
